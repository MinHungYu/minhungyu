...
```{r ans1b4}
str_which(classSurvey$`學號`,"4104d245c7")
```
...
```{r ans12}
a<-str_detect(classSurvey$`課外活動`,"團隊活動")
sum(a,na.rm = T)
```
...
```{r ans16}
(hwGrades$`學號` %in% classSurvey$`學號`) -> t
hwGrades[t,] -> hwGradesSubsample
```
...
hwGradesSubsample$`學號` %in% `女性學號` -> hwGradesSubsample$`女性`
```

## 1.8 性別與學業表現：男性
依**hwGradesSubsample**的**平均成績**及**女性**變數來記算出男性的「平均」**平均成績**。(最後一行必需出現此平均值)
```{r ans18}
hwGradesSubsample$`平均成績`[(hwGradesSubsample$`女性`== F)] %>% mean
```
...
```{r ans19}
classSurvey$`學號` -> `有團隊`
hwGradesSubsample$`有團活` <- hwGradesSubsample$`學號` %in% `有團隊`
hwGradesSubsample$`平均成績`[(hwGradesSubsample$`女性`==T & hwGradesSubsample$`有團隊`==T)] %>%
...
```{r ans22}
ymd_hms("2018-11-06 00:00:00",tz="Asia/Taipei") -> deadline3
